# Named expressions you can reuse elsewhere in your configuration as local.aliases and local.name_prefix
# Terraform computes them during plan/apply
locals {
  # If use_custom_domain is true and root_domain is not an empty string
  # E.g. if var.root_domain = "example.com", then aliases = ["example.com", "www.example.com"]
...
resource "aws_iam_role" "lambda_role" {
  ...
  # Also required by that CI role: its iam:CreateRole allow has an iam:PermissionsBoundary condition, so creating a role without exactly this boundary is denied. The policy itself is created by ../terraform-bootstrap (aws_iam_policy.ci_managed_role_boundary).
  permissions_boundary = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:policy/ci-managed-role-boundary"
